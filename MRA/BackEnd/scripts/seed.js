...
async function seedDatabase() {
    try {
        ...
        // Create Landlords
        const landlords = [];
        for (let i = 0; i < 5; i++) { // Reduced number of landlords for faster seeding
        ...
        // Create Tenants
        const tenants = [];
        for (let i = 0; i < 5; i++) { // Reduced number of tenants for faster seeding
        ...
        for (const landlord of landlords) {
            for (let i = 0; i < faker.number.int({ min: 1, max: 3 }); i++) { // Each landlord lists 1-3 properties
                const bedroomCount = faker.number.int({ min: 1, max: 5 }); // Random bedrooms 1 to 5
                const bathroomCount = faker.number.int({ min: 1, max: 3 }); // Random bathrooms 1 to 3
                const rentExpiryDate = faker.date.future({ years: 1 }).toISOString().split('T')[0]; // Generate expiry date
                const isVerified = faker.datatype.boolean(); // Randomly verified or not

                const property = await query(
                    `INSERT INTO properties (landlord_id, title, description, location, price, lease_duration_months, is_occupied, ownership_certificate_token, rent_expiry_date, verified, bedroom_count, bathroom_count, created_at, updated_at)
                    ...
                    [
                        ...
                        faker.datatype.boolean(), // Randomly occupied or not
                        faker.string.alphanumeric(20),
                        rentExpiryDate, // Parameter $9
                        isVerified,     // Parameter $10
                        bedroomCount,   // Parameter $11
                        bathroomCount   // Parameter $12
                    ]
                );
                properties.push(property.rows[0]);

                // Add multiple images for each property (6 to 8 images)
                for (let j = 0; j < faker.number.int({ min: 6, max: 8 }); j++) {
                    await query(
                        'INSERT INTO property_images (property_id, image_url) VALUES ($1, $2);',
                        [property.rows[0].id, faker.image.urlLoremFlickr({ category: 'house', width: 640, height: 480, randomize: true })]
                    );
                }
            }
        }
        ...
        // 5. Create Admin Verifications for some properties (kept as is)

...
router.get('/', authenticateToken, async (req, res) => {
    ...
    let queryText = `
        SELECT
            ...
            p.rent_expiry_date,
            p.verified,
            p.bedroom_count, -- Added bedroom_count
            p.bathroom_count, -- Added bathroom_count
    ...
    try {
        ...
        const properties = propertiesResult.rows.map(p => ({
            ...
            verified: p.verified,
            bedroomCount: p.bedroom_count, // Added to response
            bathroomCount: p.bathroom_count // Added to response
        }));
    ...
    try {
        const propertyResult = await query(
            `SELECT p.id, p.title, p.description, p.location, p.price, p.lease_duration_months,
                    p.is_occupied, p.rent_expiry_date, p.verified, p.bedroom_count, p.bathroom_count, -- Added bedroom_count and bathroom_count
        ...
        res.status(200).json({
            ...
            bedroomCount: property.bedroom_count, // Added to response
            bathroomCount: property.bathroom_count, // Added to response
            owner: {
